...
const beamSupportRect = document.getElementById("beamSupportRect");
const beamSupportLine = document.getElementById("beamSupportLine");
const beamHolder = document.getElementById("beamHolder");
...
const beemlenght = 4.6;
const beamSupportLength = 2;
const stringLength = 5;
const deflMeterRadius = 55;
const dlefMeterAngles = [0,75,150];


const beamSuppRect = {width:60,height:50};
const suppStand = {width:50,height:50};
const weightHang = {width:10,height:20};

const margin = 10;
const xStart = margin;
const yStart = 200;
const xEnd = xStart + beemlenght * 100;
const yEnd = yStart + beamSupportLength * 100 + beamSuppRect.height;
const SVG = {

              width : margin + beamSuppRect.width / 2 + beemlenght * 100 + deflMeterRadius,
              height : yEnd + 10
...
const drawMainScene = () => {
  ...
  beamSupportRect.setAttribute("x",xStart);
  beamSupportRect.setAttribute("y",yEnd - beamSuppRect.height);
  beamSupportRect.setAttribute("width",beamSuppRect.width);
  beamSupportRect.setAttribute("height",beamSuppRect.height);

  beamSupportLine.setAttribute("x1",xStart + beamSuppRect.width/2);
  beamSupportLine.setAttribute("y1",yStart-40);
  beamSupportLine.setAttribute("x2",beamSupportLine.getAttribute("x1"));
  beamSupportLine.setAttribute("y2",yStart + beamSupportLength * 100);

  beamHolder.setAttribute("x",xStart + beamSuppRect.width/2-10);
  beamHolder.setAttribute("y",yStart-10);
  beamHolder.setAttribute("width",20);
  beamHolder.setAttribute("height",20);

  d=["M", xStart + beamSuppRect.width/2, yStart,
    "C",xStart+100,yStart,xEnd - 150,yStart,xEnd+5,yStart]
  beam.setAttribute("d",d.join(" "));

  drawWeight(xEnd-8,yStart)
  drawDlefMeter(xEnd+1,yStart -(deflMeterRadius+40),deflMeterRadius,0);

}

function drawWeight(wx,wy) {
  // body...
  weightHanger.setAttribute('x',wx-(weightHang.width/2));
  weightHanger.setAttribute('y',wy-10);
  weightHanger.setAttribute('width',weightHang.width);
  weightHanger.setAttribute('height',weightHang.height);
...
function drawDlefMeter(cx,cy,radius,standdir) {
  ...
  hLine.setAttribute('x2',temp1);
  hLine.setAttribute('y2',cy);

  lineHolder.setAttribute("cx",temp2);
...
function removeWeight() {
  ...
  if(weightValue > 0) {
    ...
    setdlefReadingU(-- index);
    dlefReading.setAttribute("x",parseInt(dlefReading.getAttribute("x")) + 2);

  } else { alert("Can not Remove Weight!")}
...
function increaseWeightSize() {
  ...
  for (var i = 1; i < points.length; i+=2) {
    points[i] = parseInt(points[i]) + shift; //moves weight UP. (only Y coordinates should changed)
  }
  
  weight.setAttribute('points',points.join(" "));

  weightHanger.setAttribute("y",parseInt(weightHanger.getAttribute("y"))+6);

  string.setAttribute("y1",parseInt(string.getAttribute("y1"))+6);
  string.setAttribute("y2",parseInt(string.getAttribute("y2"))+6);

  weightText.setAttribute('y',points[7]-10);
...
function decreaseWeightSize() {
  ...
  for (var i = 1; i < points.length; i+=2) {
    points[i] = parseInt(points[i]) - shift; //moves weight down. (only Y coordinates should changed)
  }
...
const dlefL = [0,26,51,78,105,134];
const dlefU = [0,30,56,83,108,134];
const wv = [4.903,9.806,14.709,19.613,24.517];
const lodini = [0,26,51,78,105];
const lodfin = [26,51,78,105,134];
const unlodini = [30,56,83,108,134];
const unlodfin = [0,30,56,83,108];
const lodmm = [0.26,0.25,0.27,0.27,0.29];
const unlodmm = [0.30,0.26,0.27,0.25,0.26];
const ym = [1.74,1.82,1.83,1.83,1.82];
var minput = [verifyYM.ym1,verifyYM.ym2,verifyYM.ym3,verifyYM.ym4,verifyYM.ym5];
const pow10 = 5;

function setdlefReadingL(index) {
  dlefReading.innerHTML = dlefL[index];
}

function setdlefReadingU(index) {
  dlefReading.innerHTML = dlefU[index];
}
...
function showResults() {
  ...
  result.innerHTML = "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<h3><u>Result :- </u></h3>"+
                     "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Young’s modulus of beam materials is &nbsp;<u>"+
