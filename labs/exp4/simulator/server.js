...
var shift = 5;
const weightShift = 6;
var needleShift = 3; //should be an intiger

const stringLength = 5;
const weightHangerWidth = 8;
const weightHangerHeight = 12;

const beemlenght = 5.2;
const beamStroke = 8;
const supportLength = 3.5;
const deflMeterRadius = 55;
const dlefMeterAngles = [0,40,80];
...
const drawMainScene = () => {
  ...
  drawDlefMeter(xPositions[1],yStart -(deflMeterRadius+40),deflMeterRadius,0);
...
function drawWeight(wx,wy) {
  ...
  weightHanger.setAttribute('y',wy-weightHangerHeight/2);
...
function drawDlefMeter(cx,cy,radius,standdir) {
  ...
  deflReading.setAttribute("font-size","15px");
  ...
  vLine.setAttribute('y1',parseInt(lineHolder.getAttribute("cy") - 30));
...
function changePositions() {
  ...
  if(state == 0) {
    ...
    drawDlefMeter(xPositions[0],yStart -(deflMeterRadius+40),deflMeterRadius,1);
    state = 1;
...
const lac = [4.903,9.806,14.709,19.613,24.517];;
const da4 = [0,15,32,49,65];
const la4 = [4.903,9.806,14.709,19.613,24.517];;
const dac = [0,15,31,50,67];
const p1 = [0.7357,3.1392,7.2103,12.7533];
const p2 = [0.7357,3.1392,7.3575,13.1454];
const minput1 = [verifyP.p11,verifyP.p12,verifyP.p13,verifyP.p14];
const minput2 = [verifyP.p21,verifyP.p22,verifyP.p23,verifyP.p24];
...
function updateObserTable() {
  ...
  if(state == 0) {
    ...
    if(!wSet0.has(weight)) {
      wSet0.add(weight);
      obserTable.rows[weight*2].cells[1].innerHTML= lac[weight*2 - 1];
      obserTable.rows[weight*2].cells[2].innerHTML= ""+da4[weight * 2]+"&nbsp;x&nbsp;0.01 =&nbsp;"+da4[weight*2] * 0.01;
    ...
    if(!wSet1.has(weight)) {
      wSet1.add(weight);
      obserTable.rows[weight*2].cells[3].innerHTML= la4[weight*2 - 1];
      obserTable.rows[weight*2].cells[4].innerHTML= ""+dac[weight * 2]+"&nbsp;x&nbsp;0.01 =&nbsp;"+dac[weight*2] * 0.01;
...
function showResults() {
  ...
  average.innerHTML = "&nbsp;&nbsp;Average of P<sub>1</sub>&delta;<sub>12</sub> <br> "+
                      "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; = ( ";
  average.innerHTML += p1[0];
  average1 += p1[0];
  for (var i = 1; i < p1.length; i++) {
    average.innerHTML += " + "+p1[i];
    average1 += p1[i];
  }
  average.innerHTML += ") / " + (p1.length) + " = " + average1 + "." + "<br>";

  average.innerHTML += "&nbsp;&nbsp;Average of P<sub>2</sub>&delta;<sub>21</sub> <br>" + 
                      "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; = ( ";
  average.innerHTML += p2[0];
  average2 += p2[0];
  for (var i = 1; i < p2.length; i++) {
    average.innerHTML += " + "+p2[i];
    average2 += p2[i];
  }
  average.innerHTML += ") / " + (p2.length) + " = " + average2 + ".";

  result.innerHTML = "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<h3><u>Result :- </u></h3>"+
                     "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" +
                     "P<sub>1</sub>&delta;<sub>12</sub> <u>~</u> P<sub>2</sub>&delta;<sub>21</sub><br>"+
                     "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" +
                     "Hence Maxwell's reciprocal theorem on beams is Proved.";
...
function showResultTable() {
  ...
  for (var i = 0; i < minput1.length; i++) {
    ...
    if(minput1[i].value == "") {
      ...
      measured1.push(parseFloat(minput1[i].value))
    }

    if(minput2[i].value == "") {
      measured2.push(0);
    } else {
      measured2.push(parseFloat(minput2[i].value))
    }
  ...
  for (var i = 0; i < p1.length; i++) {
      ...
      resultTable.rows[2 + i].cells[1].innerHTML = p1[i];
      resultTable.rows[2 + i].cells[2].innerHTML = minput1[i].value;
      resultTable.rows[2 + i].cells[3].innerHTML = percentageError(measured1[i],p1[i]);

      resultTable.rows[2 + i].cells[4].innerHTML = p2[i];
      resultTable.rows[2 + i].cells[5].innerHTML = minput2[i].value;
      resultTable.rows[2 + i].cells[6].innerHTML = percentageError(measured2[i],p2[i]);
